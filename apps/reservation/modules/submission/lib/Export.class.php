        ...
        $stmt = $con->query("SELECT ASV.AmendmentId AS Id, ASV.SubmissionId AS SubmissionId, ASV.SubmissionNumber AS SubmissionNumber,ASV.DuckSubmissionNumber AS DuckSubmissionNumber,ASV.FinalPolicyNumber AS MasterPolicyNumber, ASV.NewRenewal AS NewRenewal, ASV.InsuredName AS InsuredName, ASV.AdvisenId AS AdvisenId,ASV.SubmissionIdentifier AS SubmissionIdentifier,
                        ...
                        left join
                           (select s.id sid, 
                            ...
                            on s.id=r.ksubmissionid group by s.id) b on ASV.SubmissionId = b.sid
    ...
    public function exportQCCSV($arrWhere) {
        ...
        if (!empty($count->COUNT)) {
            $stmt = $con->query("SELECT SV.SubmissionId AS Id, SV.SubmissionNumber AS SubmissionNumber,SV.DuckCreekSubmissionNumber AS DuckCreekSubmissionNumber,SV.MasterPolicyNumber AS MasterPolicyNumber, SV.NewRenewal AS NewRenewal, SV.InsuredName AS InsuredName, SV.AdvisenId AS AdvisenId, SV.ReasonCodeMeaning AS ReasonCodeMeaning, SV.UnderWriterName AS Underwriter, SV.PropertyType AS ProductLine, SV.PropertyLineSubType AS ProductLineSubType, SV.SectionCode AS Section, SV.ProfitCode AS ProfitCode, 
                                SV.ISOCode AS ISOCode,SV.CurrentStatus AS CurrentStatus, CONVERT(INT, CONVERT(DATETIME,SV.EffectiveDate))+2 AS EffectiveDate, CONVERT(INT, CONVERT(DATETIME,SV.ExpiryDate))+2 AS ExpiryDate, CONVERT(INT, CONVERT(DATETIME,SV.ProcessDate))+2 AS ProcessDate, CONVERT(INT, CONVERT(DATETIME,SV.BindDate))+2 AS BindDate, SV.Renewable AS Renewable, CONVERT(INT, CONVERT(DATETIME,SV.DateOfRenewal))+2 AS DateOfRenewal,
