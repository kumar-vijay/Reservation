...
class IndividualBrokerReport {
    ...
    public function getReport($postArray) {
        define("REPORT", "/KPI Reports As Of Date/Individual Broker Report");
        ...
        try {
            ...
            $parameters[5] = new ParameterValue();
            $parameters[5]->Name = "brokername";
            $parameters[5]->Value = "" . $postArray['brokername'] . "";
    ...
    public function getStartDateData($daterange) {
        ...
        $query = "SELECT StartDate,EndDate FROM vw_ReportRange WHERE ReportRange = '$daterange'";
    ...
    public function DownloadReport($postArray, $reportType) {
        ...
        try {
            ...
            if ($reportType == 'PDF') {
                ...
                header("Content-Disposition:attachment;filename='Individual-Broker-Report.pdf'");
                ...
                $renderAsEXCEL->PageWidth = "12.5in";
                ...
                header("Content-Disposition:attachment;filename=Individual-Broker-Report.xls");
                header("Content-length: " . (string) (strlen($result)));
                header("Expires: " . gmdate("D, d M Y H:i:s", mktime(date("H") + 2, date("i"), date("s"), date("m"), date("d"), date("Y"))) . " GMT");
                header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
                header("Cache-Control: no-cache, must-revalidate");
                ...
                header("Content-Disposition:attachment;filename='Individual-Broker-Report.doc'");
                header("Content-length: " . (string) (strlen($result)));
                header("Expires: " . gmdate("D, d M Y H:i:s", mktime(date("H") + 2, date("i"), date("s"), date("m"), date("d"), date("Y"))) . " GMT");
                header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
                header("Cache-Control: no-cache, must-revalidate");
