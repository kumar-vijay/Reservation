...
class AccountMultipleLines {
    ...
    public function getReport($postArray) {
        define("REPORT", "/KPI Reports As Of Date/Accounts on Multiple Lines");
        ...
        try {
            ...
            $result_html .= '<div>';
            $result_html .= '<script>document.getElementById("AccountsMultipleLinesForm").classList.add("inputFix");</script>';
    ...
    public function DownloadReport($postArray, $reportType) {
        ...
        try {
            ...
            if ($reportType == 'PDF') {
                ...
                header("Content-Disposition:attachment;filename='Account-on-Multiple-Lines.pdf'");
                ...
                header("Content-Disposition:attachment;filename='Account-on-Multiple-Lines.csv'");
                header("Content-length: " . (string) (strlen($result)));
                header("Expires: " . gmdate("D, d M Y H:i:s", mktime(date("H") + 2, date("i"), date("s"), date("m"), date("d"), date("Y"))) . " GMT");
                header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
                header("Cache-Control: no-cache, must-revalidate");
                ...
                header("Content-Disposition:attachment;filename=Account-on-Multiple-Lines.xls");
                header("Content-length: " . (string) (strlen($result)));
                header("Expires: " . gmdate("D, d M Y H:i:s", mktime(date("H") + 2, date("i"), date("s"), date("m"), date("d"), date("Y"))) . " GMT");
                header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
                header("Cache-Control: no-cache, must-revalidate");
                ...
                header("Content-Disposition:attachment;filename='Account-on-Multiple-Lines.doc'");
                header("Content-length: " . (string) (strlen($result)));
                header("Expires: " . gmdate("D, d M Y H:i:s", mktime(date("H") + 2, date("i"), date("s"), date("m"), date("d"), date("Y"))) . " GMT");
                header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
                header("Cache-Control: no-cache, must-revalidate");
