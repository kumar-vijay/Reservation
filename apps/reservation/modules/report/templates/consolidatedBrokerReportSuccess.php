<?php $niddle = $sf_user->getAttribute('searchNiddle');
    foreach ($niddle['brokerType'] as $data){
        $brokers[] = $data;
    }
...
<div class="breadcrumbs group"> 
    <ul id="breadcrumb">
        ...
        <li class="selected">Accounts Summary</li>
    </ul>
...
<div class="container">
    <div class="box">
        <h1 class="section-header">Consolidated Broker Report
            <div class="arrow"></div>
        </h1>
        <div class="content" style="display: block;">
            ...
            ?>
            <form method="POST" action="" name="ConsolBrokerReportForm" id="ConsolBrokerReportForm">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                       ...
                       <td width="12%">Broker Type<span style="color: red;"> *</span></td>
                        
                        <td width="22%">
                            <div class="dropdown divdropdown"><input readonly="readonly" type='text' name="brokerVal" class='selectboxClass' id="brokerVal" value=""  style="font-size:8pt; width:130px; padding-right: 20px;" />
                                <ul class="dropdown-list">                                    
                                    <li> 
                                        <label><?php if($niddle['selectAllBroker'] == 'y') {$check = 'checked="checked"';} ?><input type="checkbox" value="y" id="selectAllBroker" name="selectAllBroker" <?php echo $check;?> />-Select All-</label>
                                    </li>                                    
                                    <?php  foreach ($parameter[6]['ValidValues'] as $value) { 
                                        if (in_array($value->Value ,$brokers))
                                        $select = 'checked="checked"';
                                        else
                                        $select = ''; 
                                    ?> 
                                    <li>
                                        <label><input class="checkBroker" type="checkbox"  <?php echo $select; ?> value="<?php echo $value->Value; ?>" name="brokerType[]" /><?php echo $value->Label; ?></label>
                                    </li>
                                    <?php } ?>
                                </ul>
                            </div>
                    ...
                    <tr>
                        ...
                        <td width="22%">
                            <select name="lineofbusiness" id="lineofbusiness" class="listbox-small">
                                <?php
                                foreach ($parameter[8]['ValidValues'] as $value) {
                                    if ($value->Value == $niddle['LineOfBusiness'])
                                        $select = 'checked="checked"';
                                    else
                                        $select = '';
                                ?> 
                                    <option <?php echo $select; ?> value="<?php echo $value->Value; ?>"><?php echo $value->Label; ?></option>
                                <?php } ?>
                            </select>
                    ...
                    <tr>
                        <td colspan="6" align="center">
                            <input type="submit" value="View Report" class="btn btn-blue" id="citySubmit" name="consolBrokerSubmit" />
...
<?php if($result){ ?>
        <div class="container">
            ...
            <form method="POST" action="/report/DownloadConsolBrokerReport" name="ConsolBrokerReportForm" id="ConsolBrokerReportForm">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
