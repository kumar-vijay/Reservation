...
abstract class BaseProducts extends BaseObject  implements Persistent {
	...
	public function setProductId($v)
	{
		...
		if ($this->product_id !== $v) {
			$this->product_id = $v;
			$this->modifiedColumns[] = ProductsPeer::PRODUCT_ID;
		}
	...
	public function setProductName($v)
	{
		...
		if ($this->product_name !== $v) {
			$this->product_name = $v;
			$this->modifiedColumns[] = ProductsPeer::PRODUCT_NAME;
		}
	...
	public function setSubmissionType($v)
	{
		...
		if ($this->submission_type !== $v) {
			...
			$this->modifiedColumns[] = ProductsPeer::SUBMISSION_TYPE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->product_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = ProductsPeer::NUM_COLUMNS - ProductsPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseProducts:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ProductsPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseProducts:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseProducts:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseProducts:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ProductsPeer::doInsert($this, $con);
					...
					$affectedRows += ProductsPeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProductsPeer::PRODUCT_ID)) $criteria->add(ProductsPeer::PRODUCT_ID, $this->product_id);
		if ($this->isColumnModified(ProductsPeer::PRODUCT_NAME)) $criteria->add(ProductsPeer::PRODUCT_NAME, $this->product_name);
		if ($this->isColumnModified(ProductsPeer::SUBMISSION_TYPE)) $criteria->add(ProductsPeer::SUBMISSION_TYPE, $this->submission_type);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ProductsPeer::PRODUCT_ID, $this->product_id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseProducts:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseProducts::%s', $method));
	  }
