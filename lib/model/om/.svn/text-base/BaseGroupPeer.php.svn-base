...
abstract class BaseGroupPeer {
	...
	const CREATED_ON = 'GROUP.CREATED_ON';

	/** the column name for the MODIFIED_ON field */
	const MODIFIED_ON = 'GROUP.MODIFIED_ON';

	/** the column name for the CREATED_BY_ID field */
	const CREATED_BY_ID = 'GROUP.CREATED_BY_ID';

	/** the column name for the MODIFIED_BY_ID field */
	const MODIFIED_BY_ID = 'GROUP.MODIFIED_BY_ID';
	...
	{
		$criteria->addSelectColumn(GroupPeer::GROUP_ID);
		$criteria->addSelectColumn(GroupPeer::GROUP_NAME);
		$criteria->addSelectColumn(GroupPeer::STATUS);
		$criteria->addSelectColumn(GroupPeer::CREATED_ON);
		$criteria->addSelectColumn(GroupPeer::MODIFIED_ON);
		$criteria->addSelectColumn(GroupPeer::CREATED_BY_ID);
		$criteria->addSelectColumn(GroupPeer::MODIFIED_BY_ID);
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseGroupPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseGroupPeer', $values, $con))
      {
		...
		if ($criteria->containsKey(GroupPeer::GROUP_ID) && $criteria->keyContainsValue(GroupPeer::GROUP_ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.GroupPeer::GROUP_ID.')');
		}
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseGroupPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseGroupPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseGroupPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseGroupPeer', $values, $con))
      {
        return $sf_hook_retval;
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(GroupPeer::GROUP_ID);
			$selectCriteria->add(GroupPeer::GROUP_ID, $criteria->remove(GroupPeer::GROUP_ID), $comparison);
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseGroupPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseGroupPeer', $values, $con, $ret);
    }
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			...
			$criteria->add(GroupPeer::GROUP_ID, (array) $values, Criteria::IN);
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		$criteria->add(GroupPeer::GROUP_ID, $pk);
	...
	public static function retrieveByPKs($pks, PropelPDO $con = null)
	{
		...
		if (empty($pks)) {
			...
			$criteria = new Criteria(GroupPeer::DATABASE_NAME);
			$criteria->add(GroupPeer::GROUP_ID, $pks, Criteria::IN);
