$(document).ready(function (e) {
            ...
            $.ajax('/submission/getCoverage', {
                ...
                'success': function (data, status, xhr) {
                    if (data.length) {
                        ...
                        $('#editamendmentcoverage').html(options);
                        if ($('#hiddenCoverage').length) {
                            var selectedCoverage = $('#hiddenCoverage').val();
                            $('select[name="editamendmentcoverage"]').find('option[value="' + selectedCoverage + '"]').attr("selected", true);
    ...
    $('#insured_name_submit').click(function (ev) {
        var inputString = $('#editamendmentinsuredname').val();
        var induredId = $('#insuredId').val();
        if (inputString.length <= 2) {
            $('.modal-container').removeClass('dp-block');
            $('body').removeClass('body-locked');
            $('#editamendmentinsuredname').valid();
        } else {
            $.ajax({
        ...
        if (inputString > 0) {
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    $('#editamendmentinsuredname').val(msg.insuredName);
    ...
    var getInsuredContactPerson = function () {
        $('#editamendmentinsuredContactPersonEmail').val("");
        $('#editamendmentinsuredContactPersonNumber').val("");
        $('#editamendmentinsuredContactPersonMobile').val("");
        var InsuredId = $('#insuredId').val();
        if (InsuredId) {
            $('#editamendmentinsuredContactPerson').prop('disabled', false);
            var dataObj = {
                'header': {
            ...
            $.ajax('/submission/getInsureContactPerson', {
                ...
                'type': 'post',
                'success': function (data, status, xhr) {
                    $('#editamendmentinsuredContactPerson').html('<option value ="0">--Select--</option>');
                    var contactPersonList = createContactPersonOption(data);
                    $('#editamendmentinsuredContactPerson').html(contactPersonList);
                }
            });
        }
    };
    ...
    var getBrokerContactPerson = function () {
        $('#editamendment_broker_contact_person_email').val("");
        $('#editamendment_borker_contact_peson_number').val("");
        $('#editamendment_borker_contact_peson_mobile').val("");
    ...
    /****************************************************************************************/
    $('#editamendmentinsuredContactPerson').change(function (ev) {
        $('#editamendmentinsuredContactPersonEmail').val("");
        $('#editamendmentinsuredContactPersonNumber').val("");
        $('#editamendmentinsuredContactPersonMobile').val("");
        var val = $(this).val();
        if (val == '0') {
            ...
            $.ajax('/submission/getInsureContactPersonInformation', {
                ...
                'success': function (data, status, xhr) {
                    if (data.length) {
                        $('#editamendmentinsuredContactPersonEmail').val(data[0]['contactPersonEmail']);
                        $('#editamendmentinsuredContactPersonNumber').val(data[0]['contactPersonPhone']);
                        $('#editamendmentinsuredContactPersonMobile').val(data[0]['contactPersonMobile']);
                    }
                }
            });
        }
    ...
    $('#broker_contact_person').change(function (ev) {
        ...
        if (val == '0') {
            ...
            $.ajax('/submission/getInsureContactPersonInformation', {
                ...
                'success': function (data, status, xhr) {
                    if (data.length) {
                        $('#editamendment_broker_contact_person_email').val(data[0]['contactPersonEmail']);
                        $('#editamendment_borker_contact_peson_number').val(data[0]['contactPersonPhone']);
                        $('#editamendment_borker_contact_peson_mobile').val(data[0]['contactPersonMobile']);
                    }
                }
            });
        }
    ...
    /****************************************************************************************/
    $('#editamendmentinsuredname').keyup(function (ev) {
        $('#insured_address').val("");
        $('#insured_country').val("");
        $('#insured_state').val("");
        $('#insured_city').val("");
        $('#insured_zipcode').val("");
        $('#db_number').val("");
        $('#editamendmentinsuredContactPerson').html('<option>--Select--</option>');
        $('#editamendmentinsuredContactPersonEmail').val("");
        $('#editamendmentinsuredContactPersonNumber').val("");
        $('#editamendmentinsuredContactPersonMobile').val("");
    });
    ...
    $('#productline_master').change(function (ev) {
        $('#editamendmentproductlinesubtype_master').html('<option value = "0">--Select--</option>');
        $('#editamendmentsection_master').html('<option value = "0">--Select--</option>');
        $('#editamendmentprofitcode_master').html('<option value = "0">--Select--</option>');
        var val = $(this).val();
        if (val == '0') {
            return false;
        }
        ...
        $.ajax('/submission/getSubmissionTypeForMaster', {
            ...
            'success': function (data, status, xhr) {
                if (data.error) {
                    $('#editamendmentproductlinesubtype_master').html('<option value="NA">Not Applicable</option>');
                } else {
                    var options = createProductLineSubTypeOption(data);
                    $('#editamendmentproductlinesubtype_master').html(options);
                    if ($('#productLineSubTypeHiddenForMaster').length) {
                        var selectedSubmissionSubType = $('#productLineSubTypeHiddenForMaster').val();
                        $('#editamendmentproductlinesubtype_master').val(selectedSubmissionSubType);
                        $('#editamendmentproductlinesubtype_master').trigger('change');
    ...
    /****************************************************************************************/
    $('#editamendmentproductlinesubtype_master').change(function (ev) {
        loadCoverage();
        $('#editamendmentsection_master').html('<option value = "0">--Select--</option>');
        $('#editamendmentprofitcode_master').html('<option value = "0">--Select--</option>');
        var val = $(this).val();
        ...
        $.ajax('/submission/getSectionCodeForMaster', {
            ...
            'success': function (data, status, xhr) {
                ...
                if (data.error) {
                    $('#editamendmentsection_master').html('<option value="722">Not Applicable</option>');
                    var dataObj = {
                        'header': {
                            'requestName': 'getProfitCodeDetailsBySubType'
                        },
                    ...
                    $.ajax('/submission/getProfitCodeDetailsBySubType', {
                        ...
                        'success': function (data, status, xhr) {
                            if (data.error) {
                                $('#editamendmentprofitcode_master').html('<option value="841">Not Applicable</option>');
                            } else {
                                $('#editamendmentprofitcode_master').html('<option>--Select--</option>');
                                var profitCodeList = createProfitCodeOption(data);
                                $('#editamendmentprofitcode_master').html(profitCodeList);
                                if ($('#profitCodeHiddenForMaster').length) {
                                    var selectedProfitCode = $('#profitCodeHiddenForMaster').val();
                                    $('#editamendmentprofitcode_master').val(selectedProfitCode);
                    ...
                    $('#editamendmentsection_master').html(options);
                    if ($('#sectionCodeHiddenForMaster').length) {
                        var selectedSectionCode = $('#sectionCodeHiddenForMaster').val();
                        $('#editamendmentsection_master').val(selectedSectionCode);
                        $('#editamendmentsection_master').trigger('change');
    ...
    /****************************************************************************************/
    $('#editamendmentsection_master').change(function (ev) {
        $('#editamendmentprofitcode_master').html('<option value = "0">--Select--</option>');
        var val = $(this).val();
        var productLineType = $('#productline_master').val();
        var productLineSubTypeID = $('#editamendmentproductlinesubtype_master').val();
        if (val == '0') {
            return false;
        }
        var dataObj = {
        ...
        $.ajax('/submission/getProfitCodeForMaster', {
            ...
            'success': function (data, status, xhr) {
                ...
                if (data.error) {
                    ...
                    $('#editamendmentprofitcode_master').html(options);
                    if ($('#profitCodeHiddenForMaster').length) {
                        var selectedProfitCode = $('#profitCodeHiddenForMaster').val();
                        $('#editamendmentprofitcode_master').val(selectedProfitCode);
                        $('#profitCodeHiddenForMaster').remove();
    ...
    $('#editamendmentclassName').change(function () {
        ...
        var editAdmitted = $('#editamendmentadmitted option:selected').text();
    ...
    var editAdmitted = $('#editamendmentadmitted option:selected').text();
    if ((!$("#editamendmentclassName").is(":disabled")) && $('#editamendmentclassName').val() > 0 && (($("#primary_status").val() == '11' || $("#primary_status").val() == '12') && editAdmitted == 'Admitted - NY FTZ')) {
        $("#editamendmentsubClass").removeAttr('readOnly');
        console.log('vijay');
        $("#editamendmentsubClass").autocomplete({
            source: function (request, response) {
